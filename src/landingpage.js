...
import phoneBackground from './img/phoneBackground.png'
...
class landingPage extends Component {
  ...
  truckEffect () {
    $(window).on('load resize scroll', function () {
      $('.bg-static').each(function () {
        ...
        var realPOS = (leftPosition * 3) + 2000
        $(this)
          .find('.bg-move')
          .css({ left: (realPOS) })
      })
  ...
  getSponsors () {
    axios.get(window.location.origin + '/assets/partners.json')
  ...
  getTeam () {
    axios.get(window.location.origin + '/assets/team.json')
  ...
  getSocialMedia () {
    axios.get(window.location.origin + '/assets/social.json')
  ...
  render () {
    return (
      <div className='App'>
        <header className={`header ${this.state.scrollTop}`}>
          <img id='logo' className='logo' src={Logo} alt='logo' />
          <div className={'burger'}>
            <input className='menu-btn' type='checkbox' id='menu-btn' />
            <label className='menu-icon' htmlFor='menu-btn'><span className='navicon' /></label>
            <ul className='menu'>
              <li><p onClick={() => scrollToComponent(this.footerDiv)} >contact us</p></li>
        ...
        <div className={'page-content'}>
          ...
          <Section className='flex-parent cash-savings' style={{ minHeight: '70vh' }}>
            <div className={'half-child center'}>
              <h1 style={{ maxWidth: '75%' }} className='heading'>Pilot test at Ekonomikum Uppsala University suggests 2/3 collection time saved.</h1>
            </div>
            <div className={'half-child'}>
              <div className='numbers'>
                <ul className={'flex-parent row'}>
                  ...
                  <li>
                    <h3 style={{ color: '#72C396' }}> <CountUp ref={countUp => { this.myCountUp = countUp }} duration={3} end={1350} /> $</h3>
                  </li>
                </ul>
                <ul style={{ color: 'gray' }} className={'flex-parent row'}>
                  ...
                  <li>
                    <h3 className='thin'><CountUp ref={countUp => { this.myCountUp = countUp }} duration={4} end={2025} /> $</h3>
                  </li>
                </ul>
                <ul style={{ color: 'gray' }} className={'flex-parent row'}>
                  <li>
                  ...
                  <li>
                    <h3 className='thin'><CountUp ref={countUp => { this.myCountUp = countUp }} duration={6} end={2345} /> $</h3>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <Section className={'flex-parent column features'} style={{ justifyContent: 'flex-start' }}>
            <h1 className='heading' style={{ padding: '1em' }}>Omicle Advantages</h1>
          ...
          {this.state.partners &&
          <Section className={'flex-parent column partners'} style={{ justifyContent: 'flex-start' }}>
            <h1 className='heading'>Partnerships built on trust</h1>
            <div className={'grid-parent'}>
              { this.state.partners.map(function (partners, i) {
                return <div key={i} className={'grid-child sponsor'}>
                  <div className={'img-container'}>
                    <img alt={partners.name} src={partners.img} />
                  </div>
                </div>
              })
              }
          ...
          {this.state.team &&
            <Section className={'flex-parent column team-members'} style={{ minHeight: '70vh' }}>
              ...
              <div className={'grid-parent'}>
                { this.state.team.map(function (profile, i) {
                  return <div key={i} className={'grid-child'}>
                    <div className={'img-container profile-img'}>
                      <img alt={profile.name} src={profile.img} />
                    </div>
                    <div><h3 className='team-name'>{profile.name}</h3></div>
                  </div>
                })
                }
              </div>
          ...
          <Footer ref={(section) => { this.footerDiv = section }}>
            <div className='information'>
              <div className='information-wrapper'>
                <p><address>
                klostergatan 10 c/o Base10<br />
                75 321 uppsala
                </address></p>
                <p className='email'>hello@<div style={{ display: 'none' }} />omicle.com</p>
                {this.state.social &&
                <div className='social-media'>
                  { this.state.social.map(function (social, i) {
                    return <SocialIcon url={social.src} />
                  })
                  }
                </div>
                }
